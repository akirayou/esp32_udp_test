...
void sensTask(void * pvParameters){
  ...
  if (sensSocket < 0) {
    ...
    while(true){
        sensBuf[0]='0';
        sendto(sensSocket, sensBuf,sizeof(sensBuf), 0, (struct sockaddr *)&sens_addr, sizeof(sens_addr));
...
void app_main()
{
    ...
    if(ESP_OK==setupWithIP()){
        while(xEventGroupGetBits( wifi_event_group )&CONNECTED_BIT)loop();
    }
